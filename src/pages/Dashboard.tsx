/**
 * Dashboard — Premium design with ONE toggle
 * Clean, modern, with smart summaries
 */
import { useState, useEffect, useCallback } from 'react';
import { 
  Power, Clipboard, Camera, XCircle, Trash2, Plus, 
  Search, Sparkles, ChevronDown, Zap, Shield, MessageSquare,
  Clock, ArrowRight, Eye, Monitor, Brain
} from 'lucide-react';
...
// ═══════════════════════════════════════════════════════════════════════════════
// Activity Item — Shows smart summaries
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] text-zinc-300 truncate group-hover:text-white transition-colors">
          {event.summary}
        </p>
...
function MemoryItem({ memory, onClick, index }: { memory: Memory; onClick: (m: Memory) => void; index: number }) {
  const sourceIcons: Record<string, { icon: React.ReactNode; color: string }> = {
    ...
    'smart-capture': { icon: <Brain size={12} />, color: 'text-emerald-400 bg-emerald-500/10' },
    ...
    <div className="rounded-2xl bg-gradient-to-br from-violet-500/[0.08] via-purple-500/[0.04] to-transparent border border-violet-500/10 overflow-hidden">
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-out ${isExpanded ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pb-4 space-y-4">
          ...
          {[
            { icon: Shield, title: '1. Local Storage', desc: 'Everything stays on your device in an encrypted SQLite database.' },
            { icon: Zap, title: '2. MCP Bridge', desc: 'AI assistants query through a local server you control.' },
            { icon: MessageSquare, title: '3. Natural Queries', desc: '"What was I working on?" — your AI understands context.' },
          ].map((step, i) => (
...
export default function Dashboard() {
  const { isActive, captureCount, events, isCapturing, toggleCapture, clearEvents } = useCaptureContext();
  ...
  // Refresh when events change (something was saved)
  useEffect(() => {
  ...
  return (
    <div className="h-full flex bg-[#09090b]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        {/* ─────────────────────────────────────────────────────────────────────
            THE BIG TOGGLE — ONE button to rule them all
            ───────────────────────────────────────────────────────────────────── */}
            ...
            <div className="flex items-center gap-4 relative">
              ...
              <div className="text-left">
                ...
                <p className="text-[12px] text-zinc-500 mt-0.5">
                  {isActive 
                    ? `${captureCount} captures · analyzing screen every 1.5s`
          ...
          {memories.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in">
              ...
              <p className="text-[13px] text-zinc-500 max-w-xs leading-relaxed">
                {isActive 
                  ? 'Smart summaries of your activity will appear here automatically!'
      ...
      {/* ════════════════════════════════════════════════════════════════════════
          ACTIVITY SIDEBAR — Shows smart summaries in real-time
          ════════════════════════════════════════════════════════════════════════ */}
          ...
          {events.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6 animate-fade-in">
              ...
              <p className="text-[10px] text-zinc-600">
                {isActive ? 'Smart summaries incoming' : 'Turn on capture to start'}
              </p>
            </div>
          ) : (
            <div className="py-1">
